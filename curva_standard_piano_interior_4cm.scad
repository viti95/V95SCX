...
// Pista base
difference() {
    pie_slice(ang=45, l=8, r=430/2, $fn=256);
    zrot(-5) pie_slice(ang=55, l=8, r=(430/2)-40, $fn=256);
    zrot_copies([45/3, 0, -45/3]) move ([195*cos(45/2),195*sin(45/2)])
    if (con_sujeccion) {
...
zrot_copies([-45 - 45/6,-45 - 45/2, -45 - (5*45/6)]) translate([-4/2,430/2-1, 3])
    union() {
...
if (con_textura) {
    tex = texture("rough");
        intersection() {
            translate ([-430/2,0,1]) yrot(90) linear_sweep(
            square([1,430/2]), texture=tex, h=430,
